...
In particular, this bound is slightly worse than our improvement over Brakerski and Shmueli's. However, one of the trace distance in their computations may have a slightly better bound, and the use of triangle inequality prevents us from getting an exact result like in the case of $\rho_{2^n,t,2^n}$. The question now becomes: can we get an exact result?
